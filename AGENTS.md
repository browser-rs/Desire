...
- **流式列表的三条滚动规则（2026-09-21，三轮实测收敛）**：① **锚点固定 `.top`**——
  内容增长绝不移动视口；`.bottom`（最初）会在流式时把视口拽回底部（用户没法上滑），
  ...
  一截）时会来回翻转。另外**流式中增长的块要固定高度 + 内部滚动**（思考块
  `ReasoningBlock` 就是），否则每段文字都会让整条会话重新排版，叠上跟随就是抖动。
