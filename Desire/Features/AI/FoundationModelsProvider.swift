...
/// `AIApproval`) and our transcript bookkeeping. Instead this provider
...
/// (`AISessionStore.processLoop`) executes it exactly like a cloud model's
...
struct FoundationModelsProvider: ModelProvider {
    ...
    private static func parseToolCall(_ json: String) -> AIToolCall? {
        ...
        return AIToolCall(
            ...
            type: "function",
            function: AIToolFunction(name: name, arguments: argsString)
        )
    }
    ...
    private static func buildInstructions(
        from messages: [AIMessage], base: String, tools: [AIToolDef]?
    ) -> String {
        var lines: [String] = []
