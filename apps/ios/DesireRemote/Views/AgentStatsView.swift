...
struct AgentStatsView: View {
    ...
    private func overviewSection(_ stats: RemoteStats) -> some View {
        Section {
            LazyVGrid(
        ...
        Section {
            ...
            if stats.unpricedTokens > 0 {
                DesireValueRow(
                    ...
                    valueColor: .orange, mono: true)
            }
            DesireValueRow(
                title: "单日峰值",
                value: DesireUI.formatTokens(stats.peakDayTokens ?? 0), mono: true)
            DesireValueRow(
                title: "最长一次对话",
            ...
            DesireSectionHeader(title: "成本")
        }
    }

    private func modelSection(_ stats: RemoteStats) -> some View {
        Section {
            ForEach(stats.models) { item in
            ...
            DesireSectionHeader(title: "按模型", subtitle: "token 与成本明细")
        }
    }
}
